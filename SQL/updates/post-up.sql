...
begin
    ...
    if not exists(select * from backend_users where login = 'demo') then
        -- Demo user
        -- DEFAULT id, which is probably 2, to trigger the backend_users_id_seq sequence update nextval()
        INSERT INTO public.backend_users(
                id, first_name, last_name, login, email, 
                password, activation_code, persist_code, reset_password_code, 
                permissions, 
                is_activated, role_id, activated_at, last_login, created_at, updated_at, deleted_at, is_superuser, metadata
                -- acornassociated_url, 
                -- acornassociated_user_user_id
            )
            VALUES(
                DEFAULT, 'Demo', '', 'demo', 'demo@example.com', 
                '$2y$10$q1.JQJXcyXhNzyvl9ivi6eA28USyu1BycOA8qyPbOZFdiJJ6E0UFe', '', NULL, '', 
                '{"cms.manage_content":-1,"cms.manage_assets":-1,"cms.manage_pages":-1,"cms.manage_layouts":-1,"cms.manage_partials":-1,"cms.manage_themes":-1,"cms.manage_theme_options":-1,"backend.access_dashboard":1,"backend.manage_default_dashboard":-1,"backend.manage_users":-1,"backend.impersonate_users":-1,"backend.manage_preferences":1,"backend.manage_editor":-1,"backend.manage_own_editor":-1,"backend.manage_branding":1,"media.manage_media":-1,"backend.allow_unsafe_markdown":-1,"system.manage_updates":-1,"system.access_logs":-1,"system.manage_mail_settings":-1,"system.manage_mail_templates":-1,"acornassociated.rtler.change_settings":1,"acornassociated.users.access_users":1,"acornassociated.users.access_groups":1,"acornassociated.users.access_settings":1,"acornassociated.users.impersonate_user":-1,"winter.location.access_settings":1,"winter.tailwindui.manage_own_appearance.dark_mode":1,"winter.tailwindui.manage_own_appearance.menu_location":1,"winter.tailwindui.manage_own_appearance.item_location":1,"winter.translate.manage_locales":1,"winter.translate.manage_messages":1,"acornassociated_location":1,"acornassociated_messaging":-1,"calendar_view":1,"change_the_past":1,"access_settings":1,"acornassociated.lojistiks.debug":-1}', 
                true, NULL, now(), NULL, now(), NULL, NULL, false, ''
            );
    end if;
end
$BODY$;
